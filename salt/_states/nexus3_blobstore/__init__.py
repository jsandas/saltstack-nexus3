...
def absent(name):
    ...
    meta = __salt__['nexus3.describe_blobstore'](name)
    exists = True
    ...
    if exists:
        ...
        delete_results = __salt__['nexus3.delete_blobstore'](name)
...
def present(name,
    ...
    if exists:
        if meta['blobstore']['softQuota'] is not None:
            ...
            if update:
                update_results = __salt__['nexus3.update_blobstore'](name, quota_type, quota_limit)
    ...
    create_results = __salt__['nexus3.create_blobstore'](name, quota_type, quota_limit, store_type)
